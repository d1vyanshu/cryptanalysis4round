...
def encrypt(plain_text, sub_keys):
  ...
  for i in range(3):
    if i == 2:
        c = right
    out = func(right,sub_keys[i])
    out = per_func(out)   #function called on right half data
  ...
  D_perm = '{0:032b}'.format(int(c, base = 2) ^ int(left, base = 2))
  D = inv_per_func(D_perm)
...
key_48bit = "222222011010101100010000111100101011110100100000"  #From 42 bit with 6 leading twos
key_56bit = ""
...
for t in p_inv:
    if t == 'x':
        ...
        key_56bit += key_48bit[t-1]

left,right = split_key(key_56bit)
...
for i in range(2**14):
    temp = '{0:014b}'.format(i)
    for indx in range(14):
        key_56bitl[miss_bits[indx]] = temp[indx]
    key_56bit = ''.join(map(str, key_56bitl))
    subkeys = gen_key(key_56bit)
    if encrypt('2000000000000000', subkeys) == '06b75a7bb26924df':
        print("success\n" + key_56bit)
        break
